...
import {
  PRODUCT_MARQUEE_IMAGES,
  PRODUCT_SLIDES,
  PRODUCTS_HUB_DEFAULTS,
} from '../products/productsHubDefaults'

function ProductCardImage({ src, fallback, alt }) {
...
function ProductMarquee({ images }) {
...
export default function Products() {
  ...
  const hub = { ...PRODUCTS_HUB_DEFAULTS, ...content }
  const slides = mergeSlides(content.slides)
  const ctaSection = { ...PRODUCTS_HUB_DEFAULTS.ctaSection, ...content.ctaSection }
  const cta = { ...PRODUCTS_HUB_DEFAULTS.cta, ...content.cta }
  ...
  return (
    <div className="w-full overflow-x-hidden bg-white text-[#111111]">
        ...
        <div className="site-container relative">
          <nav className="mb-6 text-xs text-[#5f5f5f] sm:text-sm" aria-label="Breadcrumb">
            <ol className="flex flex-wrap items-center gap-1.5">
              ...
              <li aria-hidden>&gt;</li>
              <li className="font-medium text-[#111111]">Products</li>
            </ol>
          </nav>
      ...
      <ProductMarquee images={marqueeImages} />
